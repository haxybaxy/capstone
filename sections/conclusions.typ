...
WebGPU proved capable of sustaining interactive frame rates (above 60 FPS) for up to at least $N = 50000$ particles on an Apple M2 GPU, with headless throughput of 425 timesteps per second at $N = 100000$. Browser execution via Emscripten WebAssembly adds a fixed overhead of approximately 4.3 ms per step from event-loop scheduling, yielding an overhead factor of 2.7$times$ at $N = 100000$ that diminishes as GPU compute time grows with $N$. Energy drift is numerically consistent across native and browser platforms to within 1% for $N gt.eq 10000$, confirming that the browser WebGPU implementation executes equivalent GPU computation. The overhead is additive and constant, not multiplicative: the sub-linear scaling behaviour observed in native execution is preserved in the browser, validating WebGPU as a viable platform for browser-based scientific simulation. The primary platform constraints are 32-bit arithmetic precision and the fixed per-step browser scheduling cost at small $N$, rather than buffer size limits or compute throughput.

== Contribution

This work demonstrates that WebGPU's compute shader model is sufficient for a non-trivial scientific computing workload: a complete Barnes–Hut $N$-body solver with parallel tree construction, hierarchical force evaluation, and symplectic integration. By building from a single codebase that targets both native desktop and browser environments, the implementation shows that accessible, interactive galactic dynamics simulation is achievable without specialised hardware or vendor-locked APIs. The performance characteristics reported here, together with the honest assessment of precision limitations, provide a reference point for future work on GPU-accelerated scientific computing in WebGPU as the specification and its implementations continue to mature.
