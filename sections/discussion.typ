...
The browser overhead pattern is not a simple fixed cost. Chrome is actually faster than wgpu-native at the smallest particle count, suggesting lower per-dispatch overhead in the browser's WebGPU path. But as force evaluation dominates at larger $N$, Chrome scales less efficiently: the overhead peaks around 2$times$ at moderate workloads and then gradually narrows to roughly 1.4$times$ at the largest $N$ tested. This pattern (browser faster at small $N$, slower at large $N$) implies a scaling difference in how the browser's WebGPU implementation handles the irregular memory access patterns of BVH traversal, rather than a fixed event-loop penalty. The practical implication is that browser deployment is viable for interactive $N$-body simulation at moderate particle counts, with an overhead of roughly 1.4–1.5$times$ at scientifically meaningful $N$ ($gt.eq 50000$), subject to the precision constraints discussed below @realitycheck.
