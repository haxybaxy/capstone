...
One of the simplest numerical integration schemes is the forward Euler method @Kreyszig2011. In this approach, particle positions are updated using the current velocity according to
#math.equation(
  $
...
WebGPU is a modern web standard designed to provide low-level, high-performance access to GPU hardware from within the browser. Unlike WebGL, which is built around a traditional graphics pipeline, WebGPU exposes explicit support for general-purpose GPU computation through compute shaders, storage buffers, and programmable pipelines @webgpu-gpuweb. Its design closely mirrors contemporary native graphics and compute APIs such as Vulkan @vulkan-spec, Metal @metal-spec, and Direct3D 12, enabling fine-grained control over memory layout, synchronisation, and parallel execution @usta_webgpu_2024.
...
However, WebGPU's security-focused design imposes per-operation validation and command buffer submission overhead that compounds across many small dispatches. Maczan @maczan2026 systematically characterised this overhead for LLM inference across four GPU vendors, three backends, and three browsers, finding a true per-dispatch cost of 24–36 µs on Vulkan and 32–71 µs on Metal, with substantial variation across implementations (2.2$times$ between Safari and wgpu-native on the same Metal backend). That work established that per-operation overhead, rather than kernel compute efficiency, is the dominant bottleneck for dispatch-heavy GPU workloads at small batch sizes. The present work extends this characterisation from machine learning inference to scientific simulation, where the dispatch pattern (a fixed pipeline of tree construction and force evaluation passes per timestep) differs fundamentally from the many small operations of a neural network forward pass.
